...
def image_generate_command(message):
    ...
    if prompt:
        ...
        bot.reply_to(
            ...
            "Просто опишите что вы хотите (увидеть желательно на английском).\n\n"
            "**Примеры запросов:**\n"
...
def callback_query(call):
    ...
    if call.data == "show_help":
        ...
        info_text = """
ℹ️ **MultiToolBot v1.0**
Многофункциональный бот с 15+ полезными функциями.(Альфа-версия)
...
"""
    ...
    # ==================== ПЕРЕВОДЧИК ====================
        ...
        if call.from_user.id in TextAnalysis.memory and TextAnalysis.memory[call.from_user.id]:
            ...
            bot.send_message(
                call.message.chat.id,
                "❌ Сначала введите текст для перевода.")
    ...
    # ==================== КНОПКИ ПРОДОЛЖЕНИЯ ====================
        ...
        bot.send_message(
            call.message.chat.id,
            "🌐 Введите следующий текст для перевода:")
        user_states[call.message.chat.id] = 'waiting_translate'
...
⏱️ **Утилиты:**
/timer [секунд] - Установить таймер (например: /timer 60)
...
/ai [описание] - То же самое

🎮 **Игры и развлечения:**
/games - Меню игр
/roll или /dice - Бросить кубик d6
/roll [d4/d8/d10/d12/d20/d100] - Бросить другой кубик
...
def translate_command(message):
    ...
    if text:
        ...
        bot.reply_to(message, "Введите текст для перевода после команды или просто отправьте текст:")
